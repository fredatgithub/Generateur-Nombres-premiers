...
namespace Prime_Number
{
    public partial class Win_Prime : Form
    {
        ...
        private void Btn_GenPrime_Click(object sender, EventArgs e)
        {
            ...
            int mynum;
            System.Text.StringBuilder result = new System.Text.StringBuilder();  // C'est une chaine de caractere qui vas servir à la sauvegarde du resultat et non au calcul.
        ...
        private void Btn_GenAtkin_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i <= limit; i++) myBoolArray[i] = false;
            myBoolArray[2] = true;
            myBoolArray[3] = true;
            ...
            for (int rec = 1; rec <= Racine_limit; rec++)
            {
                ...
                for (int rec2 = 1; rec2 <= Racine_limit; rec2++)
                {
                    ...
                    n = 4 * var1 + var2;
                    if ((n <= limit) && (n % 12 == 1 || n % 12 == 5)) myBoolArray[n] = !myBoolArray[n];
                    // n = 3 * x2 + y2; 
                    n -= var1;
                    if ((n <= limit) && (n % 12 == 7)) myBoolArray[n] = !myBoolArray[n];
                    // n = 3 * x2 - y2;
                    n -= 2 * var2;
                    if ((rec > rec2) && (n <= limit) && (n % 12 == 11)) myBoolArray[n] = !myBoolArray[n];
                }
            }
